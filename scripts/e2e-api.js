...
async function run() {
  ...
  try {
    ...
    const duplicateContentLanguageHeaderResponse = await sendRawHttp([
      'GET /api/health HTTP/1.1',
      `Host: 127.0.0.1:${PORT}`,
    ...
    if (!/^HTTP\/1\.1 400 /.test(duplicateContentLanguageHeaderResponse)) {
      throw new Error('expected 400 when duplicate Content-Language headers are sent');
    }
    ...
    const duplicateUserAgentHeaderResponse = await sendRawHttp([
      'GET /api/health HTTP/1.1',
      `Host: 127.0.0.1:${PORT}`,
      'User-Agent: made-my-day-e2e/1',
      'User-Agent: made-my-day-e2e/2',
      'Connection: close',
      '',
      ''
    ].join('\r\n'));
    if (!/^HTTP\/1\.1 400 /.test(duplicateUserAgentHeaderResponse)) {
    ...
    if (!/^HTTP\/1\.1 400 /.test(duplicateDateHeaderResponse)) {
    ...
    if (!/^HTTP\/1\.1 400 /.test(duplicateIfUnmodifiedSinceHeaderResponse)) {
      throw new Error('expected 400 when duplicate if-unmodified-since headers are sent');
    }

    const duplicateIfRangeHeaderResponse = await sendRawHttp([
      'GET /api/health HTTP/1.1',
      `Host: 127.0.0.1:${PORT}`,
      'If-Range: "etag-a"',
      'If-Range: "etag-b"',
    ...
    if (!/^HTTP\/1\.1 400 /.test(duplicateIfRangeHeaderResponse)) {
...
const malformedRequestIdHeaderResponse = await sendRawHttp([
    ...
    const hostOverrideHeaderResponse = await sendRawHttp([
      'GET /api/health HTTP/1.1',
      `Host: 127.0.0.1:${PORT}`,
    ...
    if (!/^HTTP\/1\.1 400 /.test(hostOverrideHeaderResponse)) {
    ...
    if (!/^HTTP\/1\.1 400 /.test(clientIpOverrideHeaderResponse)) {
    ...
    if (!/^HTTP\/1\.1 400 /.test(rangeHeaderResponse)) {
      throw new Error('expected 400 when range headers are sent');
    }
    ...
    const acceptPostDuplicateHeaderResponse = await sendRawHttp([
      'GET /api/health HTTP/1.1',
      `Host: 127.0.0.1:${PORT}`,
    ...
    if (!/^HTTP\/1\.1 400 /.test(acceptPostDuplicateHeaderResponse)) {
    ...
    if (!/^HTTP\/1\.1 400 /.test(acceptRangesDuplicateHeaderResponse)) {
    ...
    const criticalChDuplicateHeaderResponse = await sendRawHttp([
      'GET /api/health HTTP/1.1',
      `Host: 127.0.0.1:${PORT}`,
    ...
    if (!/^HTTP\/1\.1 400 /.test(criticalChDuplicateHeaderResponse)) {
      throw new Error('expected 400 when duplicate critical-ch headers are present');
    }
    ...
    if (!/^HTTP\/1\.1 400 /.test(priorityHeaderResponse)) {
      throw new Error('expected 400 when priority header is present');
    }

    const proxyStatusHeaderResponse = await sendRawHttp([
      'GET /api/health HTTP/1.1',
      `Host: 127.0.0.1:${PORT}`,
      'Proxy-Status: error=http_protocol_error',
      'Connection: close',
      '',
      ''
    ].join('\r\n'));
    if (!/^HTTP\/1\.1 400 /.test(proxyStatusHeaderResponse)) {
