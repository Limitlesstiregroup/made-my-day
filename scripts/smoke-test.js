...
try {
  ...
  const tbaOncallPlaceholderIssues = evaluateReadiness({
    MADE_MY_DAY_ONCALL_PRIMARY: 'tba',
    MADE_MY_DAY_ONCALL_SECONDARY: 'community-backup',
    MADE_MY_DAY_ESCALATION_DOC_URL: 'https://runbooks.mademyday.test/escalation'
  });
  assert.ok(
    tbaOncallPlaceholderIssues.includes('MADE_MY_DAY_ONCALL_PRIMARY must not be a placeholder value'),
    'tba placeholder on-call owner should fail release readiness'
  );

  const unsetOncallPlaceholderIssues = evaluateReadiness({
    MADE_MY_DAY_ONCALL_PRIMARY: 'unset',
    MADE_MY_DAY_ONCALL_SECONDARY: 'community-backup',
    MADE_MY_DAY_ESCALATION_DOC_URL: 'https://runbooks.mademyday.test/escalation'
  });
  assert.ok(
    unsetOncallPlaceholderIssues.includes('MADE_MY_DAY_ONCALL_PRIMARY must not be a placeholder value'),
    'unset placeholder on-call owner should fail release readiness'
  );

  const nullOncallPlaceholderIssues = evaluateReadiness({
    MADE_MY_DAY_ONCALL_PRIMARY: 'null',
    MADE_MY_DAY_ONCALL_SECONDARY: 'community-backup',
    MADE_MY_DAY_ESCALATION_DOC_URL: 'https://runbook.mademyday.test/incidents',
    MADE_MY_DAY_ADMIN_TOKEN: 'admin_token_live_1234'
  });
  assert.ok(
    nullOncallPlaceholderIssues.includes('MADE_MY_DAY_ONCALL_PRIMARY must not be a placeholder value'),
  ...
  const redactedOncallPlaceholderIssues = evaluateReadiness({
    MADE_MY_DAY_ONCALL_PRIMARY: 'redacted',
    MADE_MY_DAY_ONCALL_SECONDARY: 'community-backup',
    MADE_MY_DAY_ESCALATION_DOC_URL: 'https://runbook.mademyday.test/incidents',
    MADE_MY_DAY_ADMIN_TOKEN: 'admin_token_live_1234'
  });
  assert.ok(
    redactedOncallPlaceholderIssues.includes('MADE_MY_DAY_ONCALL_PRIMARY must not be a placeholder value'),
    'redacted placeholder on-call owner should fail release readiness'
  );
  ...
  assert.equal(isPrivateOrLocalEscalationHost('runbooks.ops.internal'), true, 'internal suffix escalation hosts should be rejected for escalation runbooks');
