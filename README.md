...
- Authorization + Idempotency-Key duplication hardening: duplicate raw `Authorization` headers on admin routes and duplicate raw `Idempotency-Key` headers on JSON mutation/admin POST APIs are rejected with HTTP 400 (`invalid authorization header` / `invalid idempotency-key header`) to prevent intermediary/header-fold ambiguity
