...
- Incoming `Host` header hardening: duplicate `Host` headers are rejected with HTTP 400, malformed/control-char/oversized or delimiter-smuggled host headers are rejected with HTTP 421 before routing, and multi-hop proxy forwarding headers (`X-Forwarded-For` / `Forwarded` values containing comma chains) are rejected with HTTP 400 when `TRUST_PROXY=true` to prevent ambiguous client-IP attribution
