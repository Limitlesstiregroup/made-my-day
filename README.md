...
- Host header duplication hardening: duplicate raw `Host` header lines are rejected with HTTP 400 to reduce intermediary/header-fold ambiguity in upstream host attribution
...
- `If-Range` syntax hardening rejects malformed validator values with HTTP 400 (`invalid if-range header`); values must be a valid HTTP-date or strong/weak ETag token to avoid ambiguous conditional range semantics across intermediaries
