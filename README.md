...
- Low-level parser/protocol rejections (`clientError`/`CONNECT`) and static/fallback 404 responses emit full security headers + no-store caching semantics for safer intermediary behavior during malformed-traffic bursts
...
- Query method-override hardening: requests carrying `_method`, `method`, `x-http-method-override`, or `x-method-override` query parameters are rejected with HTTP 400 (`query method override parameters are not allowed`) to prevent URL-based verb-tunneling bypasses across intermediary rewrites
...
- Client-certificate forwarding hardening: requests carrying `X-Forwarded-Client-Cert` are rejected with HTTP 400 (`x-forwarded-client-cert header is not allowed`) to prevent upstream identity spoofing via untrusted client-cert forwarding headers
...
- Forwarded path-override hardening: requests carrying `X-Forwarded-URI`, `X-Forwarded-Path`, or `X-Original-URI` are rejected with HTTP 400 (`forwarded path override headers are not allowed`) to prevent intermediary upstream path-attribution overrides
- Host-override header hardening: requests carrying `X-Original-Host` or `X-Host` are rejected with HTTP 400 (`host override headers are not allowed`) to prevent intermediary host-attribution override ambiguity
- Forwarded-server provenance hardening: requests carrying `X-Forwarded-Server` are rejected with HTTP 400 (`x-forwarded-server header is not allowed`) to prevent intermediary server-identity spoofing via untrusted forwarding metadata
- Forwarded-proxy identity hardening: requests carrying `X-Forwarded-By` are rejected with HTTP 400 (`x-forwarded-by header is not allowed`) to prevent untrusted intermediary identity metadata from influencing origin attribution
- Forwarded-host provenance hardening: when trusted-proxy mode is enabled, `X-Forwarded-Host` must exactly match the incoming `Host` header (else HTTP 400 `x-forwarded-host must match host header`) to prevent mismatched origin-authority attribution across intermediary hops
- Legacy proto-override hardening: requests carrying `X-Forwarded-Ssl` are rejected with HTTP 400 (`x-forwarded-ssl header is not allowed`) to prevent intermediary TLS-attribution spoofing via deprecated forwarding metadata
- Client-IP override hardening: requests carrying `X-Client-IP`, `True-Client-IP`, `X-Real-IP`, `X-Forwarded-Client-IP`, `CF-Connecting-IP`, `Fastly-Client-IP`, or `Fly-Client-IP` are rejected with HTTP 400 (`client ip override headers are not allowed`) to prevent untrusted client-ip attribution spoofing via edge-only headers
...
- CDN loop attribution hardening: requests carrying intermediary loop-detection `CDN-Loop` headers are rejected with HTTP 400 (`cdn-loop header is not allowed`) to prevent untrusted edge-loop metadata from influencing origin request handling
- Time-travel negotiation hardening: requests carrying `Accept-Datetime` are rejected with HTTP 400 (`accept-datetime header is not allowed`) to prevent unsupported Memento datetime-negotiation semantics from creating intermediary cache/proxy ambiguity on origin APIs
- Sec-CH-UA client hints duplication hardening: duplicate raw `Sec-CH-UA`, `Sec-CH-UA-Arch`, `Sec-CH-UA-Bitness`, `Sec-CH-UA-Full-Version-List`, `Sec-CH-UA-Mobile`, `Sec-CH-UA-Model`, `Sec-CH-UA-Platform`, `Sec-CH-UA-Platform-Version`, `Sec-CH-UA-WoW64`, and `Sec-CH-UA-Form-Factors`, `Sec-CH-Prefers-Color-Scheme`, `Sec-CH-Prefers-Reduced-Motion`, `Sec-CH-Prefers-Reduced-Transparency`, `Sec-CH-Prefers-Contrast`, and `Sec-CH-Forced-Colors` header lines are rejected with HTTP 400 (`invalid sec-ch-ua header`) to prevent intermediary/header-fold ambiguity in user-agent client hints attribution
- Sec-CH-UA full-version client hints duplication hardening: duplicate raw `Sec-CH-UA-Full-Version` header lines are rejected with HTTP 400 (`invalid sec-ch-ua header`) to prevent intermediary/header-fold ambiguity in UA full-version attribution
...
- Width client hints duplication hardening: duplicate raw `Width` header lines are rejected with HTTP 400 (`invalid width header`) to prevent intermediary/header-fold ambiguity in resource width client hints attribution
- Save-Data client hints duplication hardening: duplicate raw `Save-Data` header lines are rejected with HTTP 400 (`invalid save-data header`) to prevent intermediary/header-fold ambiguity in save-data client hints attribution
- Downlink client hints duplication hardening: duplicate raw `Downlink` header lines are rejected with HTTP 400 (`invalid downlink header`) to prevent intermediary/header-fold ambiguity in network downlink client hints attribution
- ECT (Effective Connection Type) client hints duplication hardening: duplicate raw `ECT` header lines are rejected with HTTP 400 (`invalid ect header`) to prevent intermediary/header-fold ambiguity in effective connection type client hints attribution
- RTT (Round-Trip Time) client hints duplication hardening: duplicate raw `RTT` header lines are rejected with HTTP 400 (`invalid rtt header`) to prevent intermediary/header-fold ambiguity in round-trip time client hints attribution
- CORS preflight header duplication hardening: duplicate raw `Access-Control-Request-Method` or `Access-Control-Request-Headers` header lines are rejected with HTTP 400 (`invalid access-control-request-method header` / `invalid access-control-request-headers header`) to prevent intermediary/header-fold ambiguity in CORS preflight handling
- Host header duplication hardening: duplicate raw `Host` header lines are rejected with HTTP 400 (`duplicate host headers are not allowed`) to reduce intermediary/header-fold ambiguity in upstream host attribution
...
- HTTP verb hardening: all non-`GET|HEAD|POST` methods are rejected with HTTP 405 + `Allow` (`GET, HEAD, POST`) to reduce unsupported verb exposure (including `TRACE`/`CONNECT`) and close reflective diagnostic/proxy-tunnel surface area
...
- Authorization value-shape hardening: comma-joined `Authorization` credential payloads (for example `Bearer ..., Basic ...`) are rejected with HTTP 400 (`invalid authorization header`) before route handling to prevent ambiguous downstream auth attribution across intermediaries
- Content-Type duplication hardening for JSON mutation/admin POST APIs: duplicate `Content-Type` header lines are rejected with HTTP 400 (`invalid content-type header`) to prevent intermediary/header-fold ambiguity
- Global Content-Type duplication hardening: duplicate raw `Content-Type` header lines are rejected with HTTP 400 (`invalid content-type header`) before route handling across all endpoints to prevent intermediary/header-fold ambiguity outside JSON mutation/admin routes
- Global Content-Encoding duplication hardening: duplicate raw `Content-Encoding` header lines are rejected with HTTP 400 (`invalid content-encoding header`) before route handling across all endpoints to prevent intermediary/header-fold ambiguity outside JSON mutation/admin routes
- Conditional validator precedence hardening now rejects requests that send both `If-None-Match` + `If-Modified-Since`, both `If-None-Match` + `If-Match`, both `If-Modified-Since` + `If-Unmodified-Since`, or both `If-Match` + `If-Unmodified-Since` with HTTP 400 (`ambiguous conditional validators are not allowed`) before route handling to eliminate intermediary/client validator precedence ambiguity ✅
- `If-Range` syntax hardening rejects malformed validator values with HTTP 400 (`invalid if-range header`); values must be a valid HTTP-date or strong/weak ETag token (wildcard `*` is rejected) to avoid ambiguous conditional range semantics across intermediaries
- ETag validator syntax hardening now rejects malformed `If-None-Match` / `If-Match` values with HTTP 400 (`invalid if-none-match header` / `invalid if-match header`) before route handling to prevent ambiguous conditional-cache/precondition parsing across intermediaries ✅
...
- Origin header hardening: duplicate raw `Origin` header lines are rejected with HTTP 400 (`invalid origin header`) before route handling to prevent intermediary/header-fold ambiguity in CORS/request-origin attribution
- Referer header hardening: duplicate raw `Referer` header lines are rejected with HTTP 400 (`invalid referer header`) before route handling to prevent intermediary/header-fold ambiguity in upstream navigation attribution
- User-Agent header hardening: duplicate raw `User-Agent` header lines are rejected with HTTP 400 (`invalid user-agent header`) before route handling to prevent intermediary/header-fold ambiguity in client attribution and request fingerprinting
...
- W3C trace-context duplication hardening: duplicate raw `traceparent` or `tracestate` header lines are rejected with HTTP 400 (`invalid traceparent header` / `invalid tracestate header`) before route handling to prevent intermediary/header-fold ambiguity in distributed tracing attribution
...
- Precondition validator hardening: duplicate raw `If-Match` header lines are rejected with HTTP 400 (`invalid if-match header`) before route handling to prevent ambiguous conditional-write validators across intermediaries
- Precondition timestamp hardening: duplicate raw `If-Unmodified-Since` header lines are rejected with HTTP 400 (`invalid if-unmodified-since header`) before route handling to prevent intermediary/header-fold ambiguity in conditional-write timestamp validators
- Conditional range-validator hardening: duplicate raw `If-Range` header lines are rejected with HTTP 400 (`invalid if-range header`) before route handling to prevent intermediary/header-fold ambiguity in partial-content/cache revalidation flows
- Cache directive duplication hardening: duplicate raw `Cache-Control` header lines are rejected with HTTP 400 (`invalid cache-control header`) before route handling to prevent intermediary/header-fold ambiguity in cache-policy attribution across intermediaries/CDNs
- Pragma directive duplication hardening: duplicate raw `Pragma` header lines are rejected with HTTP 400 (`invalid pragma header`) before route handling to prevent intermediary/header-fold ambiguity in legacy cache-directive attribution across intermediaries/CDNs
- Expires directive duplication hardening: duplicate raw `Expires` header lines are rejected with HTTP 400 (`invalid expires header`) before route handling to prevent intermediary/header-fold ambiguity in cache-freshness attribution across intermediaries/CDNs
- Surrogate-Control directive duplication hardening: duplicate raw `Surrogate-Control` header lines are rejected with HTTP 400 (`invalid surrogate-control header`) before route handling to prevent intermediary/header-fold ambiguity in CDN cache-policy attribution across intermediaries/CDNs
- Request-ID duplication hardening: duplicate raw `X-Request-Id` header lines are rejected with HTTP 400 (`invalid x-request-id header`) before route handling to prevent log-correlation ambiguity across intermediaries
- Fetch-metadata duplication hardening: duplicate raw `Sec-Fetch-Site`, `Sec-Fetch-Mode`, `Sec-Fetch-Dest`, `Sec-Fetch-User`, and `Sec-Purpose` header lines are rejected with HTTP 400 (`invalid sec-fetch-* header` / `invalid sec-purpose header`) before route handling to prevent intermediary/header-fold ambiguity in request-context and speculative-navigation attribution
...
- Optional host-header allowlist (`ALLOWED_HOSTS` or `ALLOWED_HOSTS_FILE`) to mitigate DNS rebinding and misrouted ingress (mismatches return HTTP 421); missing `Host` headers on HTTP/1.1 are rejected with HTTP 400; GA readiness requires fully-qualified DNS hostnames only (single-label names and IP literals disallowed), enforces DNS hostname max length (253 chars), rejects localhost/private-network allowlist entries, disallows duplicate host entries, and caps allowlist size at 32 entries
- Incoming `Host` header hardening: duplicate `Host` headers are rejected with HTTP 400, malformed/control-char/oversized or delimiter-smuggled host headers are rejected with HTTP 421 before routing, forwarding headers (`X-Forwarded-For`, `Forwarded`, `X-Forwarded-Host`, `X-Forwarded-Proto`, `X-Forwarded-Port`, `X-Forwarded-Prefix`, `X-Forwarded-Server`) are rejected with HTTP 400 unless `TRUST_PROXY=true`, and multi-hop forwarding headers are rejected with HTTP 400 when trusted proxy mode is enabled while duplicate forwarding header lines are rejected with HTTP 400 regardless of trusted proxy mode (with strict `host[:port]` validation for `X-Forwarded-Host`, strict `http|https` validation for `X-Forwarded-Proto`, strict numeric `1-65535` validation for `X-Forwarded-Port`, strict single-path-prefix validation for `X-Forwarded-Prefix`, and strict single-token validation for `X-Forwarded-Server`) to prevent ambiguous upstream attribution
- JSON API hardening: story mutations (`POST /api/stories`, `/api/stories/:id/like`, `/api/stories/:id/share`, `/api/stories/:id/comments`) and admin automation triggers (`POST /api/import/run`, `POST /api/hall-of-fame/run`) require a single strict JSON content type (`Content-Type: application/json` with optional `charset=utf-8`; duplicate/comma-joined values, malformed params, or non-UTF-8 charsets return HTTP 400/415), reject malformed/duplicate `Accept` headers, require `Accept` to allow JSON (`application/json`, `application/*`, or `*/*`) with HTTP 406 on mismatch, malformed JSON returns HTTP 400, and oversized payloads return HTTP 413
...
- Authorization header hardening for admin APIs: duplicate/comma-joined, whitespace-bearing, non-printable, or otherwise malformed Bearer headers are rejected with HTTP 400 before token comparison
...
- ETag-enabled JSON responses now always include `Vary: Authorization` to prevent authenticated cache-key mix-ups across intermediaries during GA traffic
- Error-response cache hardening: cached JSON helpers disable ETag/304 handling for 4xx/5xx responses and force `Cache-Control: no-store` to prevent intermediary cache poisoning/stale-failure replay during incidents
...
- HTML app shell responses (`/` + `/index.html`) now also send strict anti-cache headers to reduce stale or shared-browser caching of user-generated story content
...
- `MADE_MY_DAY_ONCALL_PRIMARY` (or `MADE_MY_DAY_ONCALL_PRIMARY_FILE`) required on-call owner for GA readiness (team handle/email/pager alias, 3-128 chars; no whitespace/control characters; URLs are rejected; quoted values like `"team-oncall"` are rejected).
- `MADE_MY_DAY_ONCALL_SECONDARY` (or `MADE_MY_DAY_ONCALL_SECONDARY_FILE`) required backup on-call owner for GA readiness (must differ from primary; 3-128 chars; no whitespace/control characters; URLs are rejected; quoted values are rejected).
- `MADE_MY_DAY_ESCALATION_DOC_URL` (or `MADE_MY_DAY_ESCALATION_DOC_URL_FILE`) required escalation runbook URL for GA readiness (must be a non-placeholder `https://` URL pointing to a specific runbook document path (root and trailing-slash directory paths are rejected), without embedded username/password credentials, without query parameters/fragments, and without non-default HTTPS ports; IP-literal hosts, single-label hosts, reserved internal suffixes such as `.local`/`.internal`/`.home.arpa`, `example.com`, `localhost`, and private-network hosts are rejected).
...
- `GET|HEAD /api/health/version` (deploy identity snapshot with app version + optional `MADE_MY_DAY_GIT_SHA`/`GIT_COMMIT_SHA` + `MADE_MY_DAY_BUILD_ID` + optional `MADE_MY_DAY_INSTANCE_ID`/`HOSTNAME`, plus runtime `nodeVersion`, process `startedAt`, `uptimeSeconds`, memory telemetry (`memoryRssBytes`, `heapUsedBytes`), CPU telemetry (`cpuUserMicros`, `cpuSystemMicros`), runtime resource telemetry (`fsReadBytes`, `fsWriteBytes`, `voluntaryContextSwitches`, `involuntaryContextSwitches`), and event-loop pressure telemetry (`eventLoopUtilization`, `eventLoopActiveMillis`, `eventLoopIdleMillis`, `eventLoopDelayMeanMillis`, `eventLoopDelayP99Millis`, `eventLoopDelayMaxMillis`) for GA incident triage)
