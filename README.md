...
- Conditional validator precedence hardening now rejects requests that send both `If-None-Match` + `If-Modified-Since` or both `If-None-Match` + `If-Match` with HTTP 400 (`ambiguous conditional validators are not allowed`) before route handling to eliminate intermediary/client validator precedence ambiguity ✅
