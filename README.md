...
- Incoming `Host` header hardening: duplicate `Host` headers are rejected with HTTP 400, malformed/control-char/oversized or delimiter-smuggled host headers are rejected with HTTP 421 before routing, and multi-hop/duplicate proxy forwarding headers (`X-Forwarded-For`, `Forwarded`, `X-Forwarded-Host`, `X-Forwarded-Proto`, `X-Forwarded-Port`) are rejected with HTTP 400 when `TRUST_PROXY=true` (with strict `host[:port]` validation for `X-Forwarded-Host`, strict `http|https` validation for `X-Forwarded-Proto`, and strict numeric `1-65535` validation for `X-Forwarded-Port`) to prevent ambiguous upstream attribution
