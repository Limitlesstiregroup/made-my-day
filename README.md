...
- Security response headers (`X-Content-Type-Options`, `X-Frame-Options`, `Referrer-Policy`, `X-DNS-Prefetch-Control`, `X-Download-Options`, `Permissions-Policy`, `X-Permitted-Cross-Domain-Policies`, `Cross-Origin-Opener-Policy`, `Cross-Origin-Resource-Policy`, `Origin-Agent-Cluster`, `Content-Security-Policy`, `X-Robots-Tag`, `Strict-Transport-Security`)
...
- Secret values loaded via `*_FILE` / `*_PREVIOUS_FILE` must use absolute paths, must not be symbolic links, and require owner-only file permissions (`chmod 600`); release readiness fails on relative paths, symlinked secret files, or when group/world permission bits are set.
