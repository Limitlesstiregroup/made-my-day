...
- `MADE_MY_DAY_ONCALL_PRIMARY` required on-call owner for GA readiness (team handle/email/pager alias, >=3 chars).
- `MADE_MY_DAY_ESCALATION_DOC_URL` required escalation runbook URL for GA readiness (must be `https://`).
