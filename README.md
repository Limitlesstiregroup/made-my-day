...
- Fetch-metadata duplication hardening: duplicate raw `Sec-Fetch-Site`, `Sec-Fetch-Mode`, `Sec-Fetch-Dest`, and `Sec-Fetch-User` header lines are rejected with HTTP 400 (`invalid sec-fetch-* header`) before route handling to prevent intermediary/header-fold ambiguity in request-context attribution
