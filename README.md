...
- Optional host-header allowlist (`ALLOWED_HOSTS` or `ALLOWED_HOSTS_FILE`) to mitigate DNS rebinding and misrouted ingress (mismatches return HTTP 421); GA readiness requires DNS hostnames only (IP literals disallowed), rejects localhost/private-network allowlist entries, and caps allowlist size at 32 entries
