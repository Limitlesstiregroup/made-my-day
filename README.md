...
- Sensitive admin/API responses now send stricter anti-cache headers (`Cache-Control: no-store, private, max-age=0` + `Pragma: no-cache` + `Expires: 0`) to reduce shared-proxy cache leakage risk
