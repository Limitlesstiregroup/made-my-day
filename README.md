...
- Client-IP override hardening: requests carrying `X-Client-IP`, `True-Client-IP`, `X-Real-IP`, `CF-Connecting-IP`, `Fastly-Client-IP`, or `Fly-Client-IP` are rejected with HTTP 400 (`client ip override headers are not allowed`) to prevent untrusted client-ip attribution spoofing via edge-only headers
