...
- Incoming `Host` header hardening: duplicate `Host` headers are rejected with HTTP 400, malformed/control-char/oversized or delimiter-smuggled host headers are rejected with HTTP 421 before routing, and multi-hop/duplicate proxy forwarding headers (`X-Forwarded-For`, `Forwarded`, `X-Forwarded-Host`, `X-Forwarded-Proto`) are rejected with HTTP 400 when `TRUST_PROXY=true` (with strict `host[:port]` validation for `X-Forwarded-Host` and strict `http|https` validation for `X-Forwarded-Proto`) to prevent ambiguous upstream attribution
- JSON API hardening: story mutations (`POST /api/stories`, `/api/stories/:id/like`, `/api/stories/:id/share`, `/api/stories/:id/comments`) and admin automation triggers (`POST /api/import/run`, `POST /api/hall-of-fame/run`) require a single `Content-Type: application/json` value (duplicate/comma-joined values return HTTP 400); malformed JSON returns HTTP 400 and oversized payloads return HTTP 413
...
- Runtime persistence hardening: stories store writes (including first-boot and corruption recovery paths) use atomic temp-file swaps to reduce corruption risk during crashes/restarts; idempotency cache writes also maintain a `.bak` fallback snapshot for restart-time corruption recovery
...
- `GET|HEAD /api/health/version` (deploy identity snapshot with app version + optional `MADE_MY_DAY_GIT_SHA`/`GIT_COMMIT_SHA` + `MADE_MY_DAY_BUILD_ID`, plus runtime `nodeVersion` and process `startedAt` for GA incident triage)
