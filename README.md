...
- Conditional-cache precedence hardening now rejects requests that send both `If-None-Match` and `If-Modified-Since` with HTTP 400 (`ambiguous conditional cache validators are not allowed`) before route handling to eliminate intermediary/client validator precedence ambiguity ✅
