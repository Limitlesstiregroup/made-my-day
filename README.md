...
- Secret values loaded via `*_FILE` / `*_PREVIOUS_FILE` must use absolute paths and owner-only file permissions (`chmod 600`); release readiness fails on relative paths or when group/world permission bits are set.
