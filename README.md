...
- Incoming `Host` header hardening: duplicate `Host` headers are rejected with HTTP 400, malformed/control-char/oversized or delimiter-smuggled host headers are rejected with HTTP 421 before routing, and multi-hop/duplicate proxy forwarding headers (`X-Forwarded-For`, `Forwarded`, `X-Forwarded-Host`, `X-Forwarded-Proto`) are rejected with HTTP 400 when `TRUST_PROXY=true` (with strict `http|https` validation for `X-Forwarded-Proto`) to prevent ambiguous upstream attribution
